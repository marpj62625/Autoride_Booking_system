    ...
    window.toggleAdminNotifPanel = function() {
        ...
        if (!isVisible) {
            ...
            if (!notifs.length) {
                list.innerHTML = '<p style="color:#64748b;text-align:center;padding:16px 0;font-size:0.875rem;">No notifications yet</p>';
            } else {
                list.innerHTML = notifs.map(n => {
                    const ts = n.created_at ? new Date(n.created_at).toLocaleString() : '';
                    const unreadStyle = n.is_read ? '' : 'border-left:3px solid #6366f1;padding-left:8px;background:rgba(99,102,241,0.08);';
        ...
        _render() {
            ...
            strip.innerHTML = this._items.map((item, i) => `
                <div style="position: relative; flex-shrink: 0;">
                    <div style="width: 80px; height: 80px; border-radius: 10px; overflow: hidden; border: 2px solid ${i === 0 ? '#6366f1' : 'rgba(255,255,255,0.1)'};">
                        <img src="${item.dataUrl}" style="width: 100%; height: 100%; object-fit: cover;">
                    </div>
                    <div style="position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.65); text-align: center; font-size: 0.55rem; font-weight: 700; color: ${i === 0 ? '#818cf8' : '#94a3b8'}; padding: 2px 0; border-radius: 0 0 8px 8px;">
                        ${item.label}
                    </div>
                ...
                <div style="display: grid; gap: 15px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div>
                            ...
                            <p style="font-weight: 700; font-size: 1rem; color: var(--text-main); margin-top: 2px;">${b.start_date ? new Date(b.start_date).toLocaleDateString() : 'N/A'} to ${b.end_date ? new Date(b.end_date).toLocaleDateString() : 'N/A'}</p>
                        </div>
                        <div>
                            <label style="color: var(--text-muted); font-size: 0.65rem; font-weight: 800; text-transform: uppercase;">Payment Status</label>
                            <p style="font-weight: 700; font-size: 1rem; margin-top: 2px; color: ${b.payment_status === 'Paid' ? 'var(--success)' : 'var(--amber)'}">${b.payment_status || 'Unpaid'}</p>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div>
                            ...
                            <p style="font-weight: 800; color: var(--success); font-size: 1.2rem; margin-top: 2px;">₱${b.total_price ? b.total_price.toLocaleString() : '0.00'}</p>
                        </div>
                        <div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <button id="mBtnAddInsp" style="padding: 8px 16px; background: #6366f1; border: none; color: white; border-radius: 10px; font-size: 0.75rem; font-weight: 700;">+ New Inspection</button>
                    </div>
                    ...
                    <div id="mInspForm" style="display: none; background: rgba(15, 23, 42, 0.4); padding: 15px; border-radius: 12px; border: 1px solid rgba(99, 102, 241, 0.3);">
                        <!-- Form content... -->
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px;">
                            <select id="mfInspType" style="padding: 10px; background: #0f172a; color: white; border: 1px solid var(--border); border-radius: 8px; font-size: 0.8rem;">
                                <option value="pickup">Pickup</option>
                                <option value="return">Return</option>
                            </select>
                            <input type="number" id="mfInspMileage" placeholder="Mileage (km)" style="padding: 10px; background: #0f172a; color: white; border: 1px solid var(--border); border-radius: 8px; font-size: 0.8rem;">
                        </div>
                        <select id="mfInspFuel" style="width: 100%; padding: 10px; background: #0f172a; color: white; border: 1px solid var(--border); border-radius: 8px; font-size: 0.8rem; margin-bottom: 10px;">
                        ...
                        <textarea id="mfInspNotes" placeholder="Condition Notes..." style="width: 100%; height: 60px; background: #0f172a; color: white; border: 1px solid var(--border); border-radius: 8px; padding: 10px; font-size: 0.8rem; margin-bottom: 10px;"></textarea>
                    ...
                    ${b.status === 'Confirmed' || b.status === 'Approved' ? `
                        <button onclick="Bookings.pickup(${b.id})" class="btn-premium" style="width: 100%; background: #7c3aed; margin-top: 10px;"><i class="fas fa-car"></i> Mark as Picked Up</button>
                    ` : ''}

                    ${b.status === 'Picked Up' ? `
                        <button onclick="Bookings.complete(${b.id})" class="btn-premium" style="width: 100%; background: #059669; margin-top: 10px;"><i class="fas fa-flag-checkered"></i> Mark as Completed</button>
                    ` : ''}

                    <div style="display: grid; grid-template-columns: 0.8fr 1.2fr 1.2fr; gap: 10px; margin-top: 20px;">
                        <button onclick="document.getElementById('bookingDetailsModal').style.display='none'" class="btn-outline" style="padding: 12px 5px; font-size: 0.85rem; background: #1e293b;">Close</button>
                        <button onclick="window.open('${API_BASE}/api/bookings/${b.id}/receipt', '_blank')" class="btn-premium" style="padding: 12px 5px; font-size: 0.85rem; background: #6366f1;">Download Receipt</button>
                        ${isCancellable ? `
                            <button onclick="Bookings.cancel(${b.id})" class="btn-outline" style="padding: 12px 5px; font-size: 0.85rem; background: #475569; border: none; color: white;">Cancel Booking</button>
                        ` : ''}
                    </div>

                    ${b.payment_status === 'Partially Paid' ? `
                        <button onclick="Bookings.markPaid(${b.id})" class="btn-premium" style="width: 100%; background: #10b981; margin-top: 10px;">Mark as Fully Paid (Cash)</button>
                    ` : ''}
                </div>
            `;
            ...
            list.innerHTML = drivers.map(d => `
                <div class="stat-card" style="padding: 18px; margin-bottom: 12px; border: 1px solid var(--border);">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 10px; border-top: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="display: flex; gap: 8px;">
                            ${d.license_document ? `<button onclick="window.open('${API_BASE}/uploads/${d.license_document}')" style="background: rgba(99,102,241,0.1); border: 1px solid var(--primary); color: var(--primary); padding: 5px 10px; border-radius: 6px; font-size: 0.7rem; font-weight: 600;">License</button>` : ''}
                            ${d.status === 'Pending' ? `
    ...
    const Verifications = {
        ...
        render() {
            ...
            list.innerHTML = this.data.map(v => {
                ...
                const detailsHtml = `
                    <div style="background:rgba(99,102,241,0.06);border:1px solid rgba(99,102,241,0.15);border-radius:12px;padding:12px 14px;margin-bottom:14px;">
                ...
                <div class="stat-card" style="padding: 20px; margin-bottom: 16px;">
                    <div style="display: flex; gap: 16px; align-items: center; margin-bottom: 14px;">
                        <div style="width: 54px; height: 54px; background: linear-gradient(135deg, #6366f1, #4f46e5); border-radius: 16px; display: flex; align-items: center; justify-content: center; font-weight: 900; color: white; font-size: 1.2rem; box-shadow: 0 4px 12px var(--primary-glow); flex-shrink:0;">
                            ${initial}
                        </div>
                        <div style="flex: 1; min-width:0;">
        ...
        render() {
            ...
            list.innerHTML = this.data.map(inst => `
                <div class="stat-card" style="padding: 15px; margin-bottom: 10px; border: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <span style="font-size: 0.55rem; padding: 3px 8px; border-radius: 8px; background: ${inst.is_active ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.05)'}; color: ${inst.is_active ? 'var(--success)' : 'var(--text-muted)'}; font-weight: 800; text-transform: uppercase;">
            ...
            list.innerHTML = this.data.map(s => `
                <div style="padding: 15px 0; border-bottom: 1px solid rgba(255,255,255,0.05);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                        ...
                        <span style="font-size: 0.6rem; padding: 4px 10px; border-radius: 10px; background: ${s.role === 'super_admin' ? 'rgba(255,255,255,0.1)' : 'rgba(251,191,36,0.1)'}; color: ${s.role === 'super_admin' ? 'white' : '#fbbf24'}; font-weight: 800; text-transform: uppercase;">
                            ${s.role.replace('_', ' ')}
                        </span>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div style="display: flex; gap: 8px;">
                            <button onclick="Staff.openModal(${s.id})" style="padding: 6px 12px; background: #334155; border: none; border-radius: 6px; color: white; font-size: 0.7rem; font-weight: 700;">Edit</button>
                            ${s.id !== currentUser.id ? `
    ...
    const GPS = {
        ...
        render() {
            ...
            list.innerHTML = this.data.map(d => `
                <div style="background: rgba(99,102,241,0.05); padding: 12px; border-radius: 12px; border: 1px solid ${this.isActive(d.last_gps_update) ? 'var(--primary-glow)' : 'var(--border)'}; margin-bottom: 10px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
    ...
    const Activity = {
        ...
        _colors: { login:'#6366f1', logout:'#94a3b8', approve:'#10b981', reject:'#ef4444', create:'#3b82f6', update:'#f59e0b', delete:'#ef4444', view:'#8b5cf6' },

        _color(action) {
            const k = Object.keys(this._colors).find(k => action.toLowerCase().includes(k));
            return k ? this._colors[k] : '#6366f1';
        },
    ...
    const ActiveNow = {
        ...
        render(allBookings) {
            ...
            list.innerHTML = active.map((b, i) => {
                ...
                return `
                <div style="background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:14px;margin-bottom:10px;border-left:3px solid #10b981;">
                    <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
                        <div style="width:40px;height:40px;border-radius:12px;background:rgba(16,185,129,0.1);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            <i class="fas fa-car" style="color:#10b981;font-size:1rem;"></i>
                        </div>
                        <div style="flex:1;min-width:0;">
                        ...
                        <span style="background:rgba(16,185,129,0.1);color:#10b981;border:1px solid rgba(16,185,129,0.25);padding:3px 8px;border-radius:20px;font-size:0.6rem;font-weight:800;flex-shrink:0;">ACTIVE</span>
                    </div>
            ...
            active.forEach(b => {
                ...
                this._countdown[b.id] = setInterval(() => {
                    ...
                    el.style.color = ms < 86400000 ? '#ef4444' : '#10b981';
                }, 1000);
            });
    ...
    async function applyPopupFilters() {
        ...
        try {
            ...
            if (currentChartType === 'revenue') {
                ...
                const pieColors = ['#6366f1','#10b981','#f59e0b','#ef4444','#3b82f6','#8b5cf6','#ec4899','#14b8a6','#f97316','#84cc16','#06b6d4','#a855f7','#e11d48','#0ea5e9','#22d3ee'];
    ...
    const UserMgmt = {
        ...
        _render(list) {
            ...
            el.innerHTML = list.map(u => {
                const verifiedBadge = u.is_verified === 2
                    ? `<span style="background:rgba(16,185,129,0.15);color:var(--success);padding:3px 8px;border-radius:20px;font-size:0.6rem;font-weight:800;">VERIFIED</span>`
                    : u.is_verified === 1
        ...
        _renderModal(u, lData) {
            ...
            <div style="text-align:center;margin-bottom:20px;">
                <div style="width:70px;height:70px;border-radius:50%;background:linear-gradient(135deg,var(--primary),#4f46e5);display:flex;align-items:center;justify-content:center;font-weight:900;font-size:1.8rem;margin:0 auto 12px;">${avatar}</div>
            ...
            <div style="background:rgba(0,0,0,0.2);border-radius:14px;padding:14px;margin-bottom:16px;">
                ...
                <button onclick="UserMgmt.resetPassword(${u.id})" style="width:100%;padding:12px;background:rgba(99,102,241,0.15);border:1px solid var(--primary);border-radius:10px;color:var(--primary-light);font-weight:700;font-size:0.85rem;">Reset Password</button>
            </div>
            ...
            <div style="background:rgba(0,0,0,0.2);border-radius:14px;padding:14px;margin-bottom:16px;">
                ...
                <button onclick="UserMgmt.toggleFreeze(${u.id}, ${u.is_frozen})" style="width:100%;padding:12px;background:${u.is_frozen ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)'};border:1px solid ${u.is_frozen ? 'var(--success)' : 'var(--danger)'};border-radius:10px;color:${u.is_frozen ? 'var(--success)' : 'var(--danger)'};font-weight:700;font-size:0.85rem;">
            ...
            el.innerHTML =
                ...
                '<div class="chat-search-container" style="margin-bottom:16px;">' +
                    ...
                    '<div class="search-results-count hidden" id="searchResultsCount"' +
                        ' style="margin-top:0.5rem;padding:0.5rem;background:rgba(99,102,241,0.1);border-radius:6px;text-align:center;">' +
        ...
        function openConversation(userId, userName) {
            ...
            el.innerHTML =
                ...
                '<div style="display:flex;gap:10px;padding-top:10px;border-top:1px solid var(--border);">' +
                    ...
                    '<button onclick="AdminChat.send()" style="background:var(--primary);color:#fff;border:none;border-radius:50%;width:42px;height:42px;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;">' +
                    ...
                    container.innerHTML = msgs.map(function(m) {
                        ...
                        if (isMe) {
                            return '<div style="display:flex;justify-content:flex-end;">' +
                                '<div style="max-width:78%;background:var(--primary);color:#fff;padding:10px 14px;border-radius:18px 18px 4px 18px;font-size:0.875rem;line-height:1.4;">' +
                                    _esc(m.message) +
    ...
    window.showAdminChatPopup = function(senderName, message, userId) {
        ...
        banner.style.cssText = 'position:fixed;top:72px;left:12px;right:12px;z-index:6000;background:var(--primary);color:#fff;border-radius:14px;padding:12px 14px;box-shadow:0 8px 24px rgba(0,0,0,0.25);display:flex;align-items:center;gap:10px;cursor:pointer;';
        banner.innerHTML =
