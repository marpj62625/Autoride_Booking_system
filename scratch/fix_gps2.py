...
path = 'admin_app/index.html'
with open(path, 'r', encoding='utf-8', errors='replace') as f:
    content = f.read()

# ---- Replace GPS tab HTML ----
gps_html_start = content.find('<!-- GPS TRACKING TAB -->')
gps_html_end = content.find('<!-- REPORTS TAB -->', gps_html_start)
if gps_html_start == -1 or gps_html_end == -1:
    print('GPS tab boundaries not found, searching differently...')
    gps_html_start = content.find('id="gps" class="tab-content"')
    print('gps tab at:', gps_html_start)
else:
    new_gps_tab = '''    <!-- GPS TRACKING TAB -->
    <div id="gps" class="tab-content" style="display: none;">
        ...
        <div class="chart-card" style="padding:0;overflow:hidden;height:400px;position:relative;margin-bottom:20px;">
            ...
            <div style="position:absolute;bottom:14px;right:14px;z-index:10;display:flex;gap:8px;">
                ...
                <button onclick="GPS.openBlynk()" style="background:rgba(15,23,42,0.85);backdrop-filter:blur(5px);border:1px solid rgba(255,255,255,0.15);color:white;padding:9px 14px;border-radius:10px;font-size:0.75rem;font-weight:600;cursor:pointer;">
    ...
    '''
    content = content[:gps_html_start] + new_gps_tab + content[gps_html_end:]
    print('GPS tab HTML replaced')

# ---- Replace GPS JS module ----
gps_js_start = content.find('// --- GPS MODULE ---')
if gps_js_start == -1:
    gps_js_start = content.find('const GPS = {')
    print('Using fallback GPS JS start at:', gps_js_start)
    # Find the closing of the GPS object
    gps_js_end = content.find('window.GPS = GPS;', gps_js_start)
else:
    gps_js_end = content.find('window.GPS = GPS;', gps_js_start)

if gps_js_start != -1 and gps_js_end != -1:
    # Find the line end after window.GPS = GPS;
    gps_js_end = content.find('\n', gps_js_end) + 1
...
'''
    ...
    print('GPS JS not found - start:', gps_js_start, 'end:', gps_js_end)
