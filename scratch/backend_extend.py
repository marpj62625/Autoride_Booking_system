...
insert_before = "@app.route('/chat/send', methods=['POST'])"
...
def extend_booking(booking_id):
    """Customer submits an extension request with payment."""
    try:
        ...
        try:
            ...
            notification_service.notify_admins_inapp(
                "Extension Request",
                "Booking #" + str(booking_id) + " - customer requests " + str(ext_days) + "-day extension. PHP " + str(float(extension_price)) + " via " + payment_method + ".",
...
def get_all_extensions():
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS booking_extensions (
                id BIGSERIAL PRIMARY KEY, booking_id INTEGER, requested_by INTEGER,
                original_end_date DATE, new_end_date DATE, extension_days INTEGER,
                extension_price NUMERIC(12,2), payment_method VARCHAR(100),
                reference_number VARCHAR(200), payment_proof_url TEXT,
...
def approve_extension(ext_id):
    """Admin approves extension - updates booking end_date."""
    try:
        cur = get_cursor()
...
def reject_extension(ext_id):
    """Admin rejects extension - refund noted for upon-return."""
    try:
        ...
        try:
            ...
            notification_service.notify_user(ext['requested_by'], "Extension Rejected",
                "Your extension for Booking #" + str(ext['booking_id']) + " was not approved. Payment of PHP " + str(float(ext['extension_price'])) + " will be refunded upon vehicle return.",
                'extension_rejected')
        except Exception:
            pass
        return jsonify({"message": "Extension rejected. Refund noted for upon-return."}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
