...
for path in ['admin_app/index.html']:
    with open(path, 'r', encoding='utf-8', errors='replace') as f:
        content = f.read()

    # Replace GPS tab HTML - find the gps tab-content div
    old_gps_tab = re.search(
        r'<!-- GPS TRACKING TAB -->\s*<div id="gps" class="tab-content".*?</div>\s*\n\s*\n',
        content, re.DOTALL
    )
    if old_gps_tab:
        new_tab = '''    <!-- GPS TRACKING TAB -->
    <div id="gps" class="tab-content" style="display: none;">
        ...
        <div class="chart-card" style="padding:0;overflow:hidden;height:400px;position:relative;margin-bottom:20px;">
            ...
            <div style="position:absolute;top:14px;left:14px;z-index:10;background:rgba(15,23,42,0.8);backdrop-filter:blur(5px);border-radius:10px;padding:7px 14px;display:flex;align-items:center;gap:8px;">
                <span id="blynkLiveIndicator" style="width:8px;height:8px;background:#00B14F;border-radius:50%;animation:pulse 1.5s infinite;display:inline-block;"></span>
            ...
            <div style="position:absolute;bottom:14px;right:14px;z-index:10;display:flex;gap:8px;">
                <button onclick="GPS.refresh()" style="background:rgba(15,23,42,0.85);backdrop-filter:blur(5px);border:1px solid rgba(255,255,255,0.15);color:white;padding:9px 14px;border-radius:10px;font-size:0.75rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:6px;">
                    <i class="fas fa-sync-alt"></i> Refresh
                </button>
                <button onclick="GPS.openBlynk()" style="background:rgba(15,23,42,0.85);backdrop-filter:blur(5px);border:1px solid rgba(255,255,255,0.15);color:white;padding:9px 14px;border-radius:10px;font-size:0.75rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:6px;">
        ...
        <div class="chart-card" style="padding:20px;margin-bottom:20px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
                <h3 style="font-size:0.9rem;font-weight:700;color:var(--text-main);display:flex;align-items:center;gap:8px;"><span style="width:3px;height:14px;background:var(--primary);border-radius:3px;display:inline-block;"></span>Vehicle Location</h3>
...
'''
        content = content[:old_gps_tab.start()] + new_tab + content[old_gps_tab.end():]
        print('GPS tab HTML replaced')
    else:
        print('GPS tab HTML not found, trying alternate search...')
        idx = content.find('id="gps" class="tab-content"')
        print('gps tab at:', idx)
...
"""
        content = content[:gps_js.start()] + new_js + content[gps_js.end():]
        print('GPS JS module replaced')
    else:
        print('GPS JS module not found via regex')
        idx = content.find('// --- GPS MODULE ---')
        print('GPS MODULE comment at:', idx)
