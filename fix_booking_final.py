...
# Let's also ensure the modal container has a white background for the header
for i, line in enumerate(new_lines):
    if 'id="bookingDetailsModal"' in line:
        new_lines[i] = '    <div id="bookingDetailsModal" class="premium-modal" style="background: rgba(15,23,42,0.9);">\n'
    if 'class="modal-content"' in line and 'padding: 25px' in line:
        new_lines[i] = '        <div class="modal-content" style="background: white; color: #0f172a; padding: 25px; border-radius: 20px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.2);">\n'
    if '<h2 style="font-size: 1.1rem; font-weight: 700; color: #0f172a; margin: 0;">Booking Details</h2>' in line:
        # Check if we should update header line
        new_lines[i-1] = '                <button onclick="document.getElementById(\'bookingDetailsModal\').style.display=\'none\'" style="position: absolute; left: 0; background: transparent; border: none; color: #0f172a; font-size: 1.2rem; padding: 5px;"><i class="fas fa-times"></i></button>\n'
        new_lines[i] = '                <h2 style="font-size: 1.1rem; font-weight: 700; color: #0f172a; margin: 0;">Booking Details</h2>\n'

with open('admin_mobile/www/index.html', 'w', encoding='latin-1') as f:
    f.writelines(new_lines)
print("Updated to perfect 3-column layout.")
